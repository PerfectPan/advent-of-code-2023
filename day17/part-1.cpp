...
int main() {
    ...
    function<int(int, int, int, int)> dij = [&](int x, int y, int dir, int cnt) -> int {
        ...
        vector<vector<vector<vector<int>>>> dis(n + 1, vector<vector<vector<int>>>(m + 1, vector<vector<int>>(4, vector<int>(4, -1))));
        vector<vector<vector<vector<int>>>> vis(n + 1, vector<vector<vector<int>>>(m + 1, vector<vector<int>>(4, vector<int>(4, 0))));
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 1; j <= 3; ++j) if (~dis[n][m][i][j]) {
